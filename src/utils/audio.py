...
def load_audio(
    ...
    try:
        ...
        if max_duration is not None:
            ...
            audio, sr = librosa.load(file_path, sr=target_sr, mono=mono, duration=max_duration)
        else:
...
def save_audio(audio: np.ndarray, file_path: str | Path, sample_rate: int = 16000) -> None:
    """
...
def extract_features_for_model(
    audio: np.ndarray, processor, sample_rate: int = 16000
) -> torch.Tensor:
    """
