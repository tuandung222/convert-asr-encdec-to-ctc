...
from transformers import AutoProcessor, AutoConfig, WhisperProcessor, WhisperFeatureExtractor
...
class ASRInferenceModel:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    ckpt_files = [os.path.join(repo_path, f) for f in os.listdir(repo_path) if f.endswith('.ckpt')]
                    if ckpt_files:
                    ...
                    raise ValueError(f"Failed to download checkpoint or repository: {str(e)} -> {str(repo_error)}")
        except Exception as e:
    ...
    def transcribe(self, audio_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            predicted_ids_filtered = [id for id in predicted_ids if id != self.processor.tokenizer.pad_token_id]
...
class ONNXASRInferenceModel(ASRInferenceModel):
    ...
    def _load_onnx_session(self, model_files):
        ...
        try:
            ...
            try:
                ...
                logger.info(f"ONNX model loaded successfully with providers: {self.ort_session.get_providers()}")
                ...
                if "_int8" in model_path:
                    ...
                    if os.path.exists(fp32_path):
                        ...
                        logger.info(f"FP32 model loaded successfully with providers: {self.ort_session.get_providers()}")
                    else:
            ...
            logger.warning("ONNX Runtime not installed. Please install with 'pip install onnxruntime'")
            raise
        except Exception as e:
            logger.error(f"Error loading ONNX model: {e}")
            raise
    ...
    def transcribe(self, audio_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            ort_inputs = {
                self.ort_session.get_inputs()[0].name: inputs.input_features
            }
    ...
    def _convert_to_onnx(self):
        ...
        try:
            ...
            if os.path.exists(int8_onnx_path):
                logger.info(f"INT8 ONNX model already exists at {int8_onnx_path}. Using existing model.")
            ...
            if os.path.exists(onnx_path):
                ...
                try:
                    ...
                    logger.warning("ONNX Runtime quantization tools not available. Using FP32 model.")
            ...
            try:
                ...
                try:
                    ...
                    quantize_dynamic(
                        ...
                        op_types_to_quantize=['MatMul'],  # Only quantize MatMul operations, not Conv
                    )
                    logger.info(f"INT8 quantization completed: {int8_onnx_path}")
