...
class ASRPerformanceTester:
    ...
    def test_single_request(self, audio_file, model="phowhisper-tiny-ctc", language="vi", repeats=5):
        ...
        for i in range(repeats):
            ...
            try:
                with open(audio_file, "rb") as f:
                    ...
                    logger.info(f"Response time: {response_time:.2f}s, RTF: {response_time / result['duration']:.2f}")
        ...
        if response_times:
            ...
            logger.info(f"Single request stats: Mean response time: {stats['mean_response_time']:.2f}s, Mean RTF: {stats['mean_rtf']:.2f}")
            return True
        else:
            logger.error("No successful requests")
            return False
    
    def test_concurrent_requests(self, audio_file, model="phowhisper-tiny-ctc", language="vi", 
                               num_concurrent=5, repeats=3):
        """Test handling of concurrent requests"""
        logger.info(f"Testing concurrent request performance with {audio_file}, {num_concurrent} concurrent requests, {repeats} times")
        ...
        for iteration in range(repeats):
            ...
            if response_times:
                ...
                logger.info(f"Iteration {iteration+1} complete: {len(successful_results)}/{num_concurrent} successful, " +
        ...
        if all_response_times:
            stats = {
                ...
                "stddev_response_time": statistics.stdev(all_response_times) if len(all_response_times) > 1 else 0
            }
            
            self.results["summary"]["concurrent_requests"] = stats
            
            logger.info(f"Concurrent request stats: Mean response time: {stats['mean_response_time']:.2f}s")
            return True
        else:
    ...
    def generate_graphs(self, output_dir="performance_results"):
        ...
        if self.results.get("single_requests"):
            ...
            response_times = [r["response_time"] for r in self.results["single_requests"] if "response_time" in r]
        ...
        if self.results.get("summary", {}).get("single_request") and self.results.get("summary", {}).get("concurrent_requests"):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--graphs-dir", default="performance_results", help="Directory for output graphs")
    parser.add_argument("--single-repeats", type=int, default=5, help="Number of repeats for single request test")
    parser.add_argument("--concurrent", type=int, default=3, help="Number of concurrent requests")
    parser.add_argument("--concurrent-repeats", type=int, default=3, help="Number of repeats for concurrent test")
    parser.add_argument("--skip-single", action="store_true", help="Skip single request tests")
    parser.add_argument("--skip-concurrent", action="store_true", help="Skip concurrent request tests")
    ...
    if not args.skip_concurrent:
        tester.test_concurrent_requests(
            ...
            repeats=args.concurrent_repeats
        )
