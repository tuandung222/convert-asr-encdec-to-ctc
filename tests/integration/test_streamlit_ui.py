...
class StreamlitUITester:
    def __init__(self, streamlit_url="http://localhost:8501", headless=True):
        ...
        self.driver = None
        self.results = {"ui_load": None, "upload_tests": [], "errors": []}
    ...
    def setup_driver(self):
        ...
        try:
            ...
            self.results["errors"].append({"stage": "setup", "error": str(e)})
            return False
    ...
    def test_ui_load(self):
        ...
        try:
            ...
            self.results["ui_load"] = {"success": True, "title": title}
            return True
        except TimeoutException:
            logger.error("Timeout waiting for UI to load")
            self.results["ui_load"] = {"success": False, "error": "Timeout waiting for UI to load"}
            self.results["errors"].append(
                {"stage": "ui_load", "error": "Timeout waiting for UI to load"}
            )
            return False
        except Exception as e:
            logger.error(f"Failed to load UI: {str(e)}")
            self.results["ui_load"] = {"success": False, "error": str(e)}
            self.results["errors"].append({"stage": "ui_load", "error": str(e)})
            return False
    ...
    def test_file_upload(self, audio_file):
        ...
        if not os.path.exists(audio_file):
            ...
            self.results["errors"].append(
                {"stage": "file_upload", "error": f"File not found: {audio_file}"}
            )
            return False
        ...
        try:
            ...
            upload_button = WebDriverWait(self.driver, 10).until(
                EC.presence_of_element_located(
                    (By.XPATH, "//button[contains(text(), 'Upload') or contains(text(), 'Browse')]")
                )
            )
            ...
            try:
                result_element = WebDriverWait(self.driver, 60).until(
                    EC.presence_of_element_located(
                        (By.XPATH, "//div[contains(@class, 'stText') and contains(text(), '')]")
                    )
                )
                ...
                self.results["upload_tests"].append(
                    {"audio_file": audio_file, "success": True, "transcription": transcription_text}
                )
                return True
            except TimeoutException:
            ...
            self.results["upload_tests"].append(
                {"audio_file": audio_file, "success": False, "error": str(e)}
            )
            self.results["errors"].append(
                {"stage": "file_upload", "error": str(e), "audio_file": audio_file}
            )
            return False
...
if __name__ == "__main__":
    ...
    tester = StreamlitUITester(streamlit_url=args.url, headless=not args.no_headless)
