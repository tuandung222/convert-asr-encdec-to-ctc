...
def test_onnx_model(audio_path, model_id, device):
    ...
    try:
        ...
        logger.info(
            f"PyTorch - Load: {pytorch_load_time:.2f}s, Inference: {pytorch_inference_time:.2f}s, Total: {pytorch_total_time:.2f}s"
        )
        logger.info(
            f"ONNX    - Load: {onnx_load_time:.2f}s, Inference: {onnx_inference_time:.2f}s, Total: {onnx_total_time:.2f}s"
        )
        logger.info(
            f"Speed improvement: {pytorch_inference_time/onnx_inference_time:.2f}x faster inference"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", type=str, default="tuandunghcmut/PhoWhisper-tiny-CTC", help="Model ID or path"
    )
    parser.add_argument(
